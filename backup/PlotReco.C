...
int PlotReco(TString Input = "./new_SFs_v8/control_eff_reco/Eff_etaphi_IDProbesWithMS_AllPt10_MediumMuons_ScaleFactor.root"){
...
//for (int i = 0; i < 1;i++){
    ...
    CanName = CanName.ReplaceAll("./new_SFs_v8/control_eff_reco/", "");
    ...
    latex = h->GetName();
    latex = latex.ReplaceAll("h_Eff_etaphi_IDProbesWithMS_AllPt10_MediumMuons_", "");
    ...
    t = new TLatex( 0.5, 3.6,"Medium WP, IDProbesWithMS");
...
for (int i = 0; i < Cans.size();i++){
    Cans[i]->SaveAs(TString::Format("./reco/%s.png", Cans[i]->GetName() ));
       Cans[i]->SaveAs(TString::Format("./reco/%s.pdf", Cans[i]->GetName() ));
}
