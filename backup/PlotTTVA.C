...
int PlotTTVA(TString Input = "./new_SFs_v8/control_eff_ttva/Eff_EtaPt_TTVA_LooseProbes_noProbeIP_AllPt10_IPCutEff_ScaleFactor.root"){
...
//for (int i = 0; i < 1;i++){
    ...
    CanName = CanName.ReplaceAll("./new_SFs_v8/control_eff_ttva/", "");
    ...
    latex = latex.ReplaceAll("h_Eff_EtaPt_TTVA_LooseProbes_noProbeIP_AllPt10_IPCutEff_", "");
    latex = latex.ReplaceAll("_", " ");
    latex = latex.ReplaceAll("rebin", "");
    latex = latex.ReplaceAll("onebin", ""); 
    ...
    t = new TLatex( 1.5, 212,"LooseProbes, noProbeIP");
    t2 = new TLatex(1.5, 202,TString::Format("%s %s", latex.Data(), ""));
    TLatex * l = new TLatex(0, 212,"#bf{#it{ATLAS}} Internal ");
    TLatex * l2 = new TLatex(0, 202,"pPb #sqrt{s}=8.16 TeV, 165 nb^{-1}");
    ...
    h_clone->GetZaxis()->SetMaxDigits(3);
...
for (int i = 0; i < Cans.size();i++){
    Cans[i]->SaveAs(TString::Format("./ttva/%s.png", Cans[i]->GetName() ));
       Cans[i]->SaveAs(TString::Format("./ttva/%s.pdf", Cans[i]->GetName() ));
}
