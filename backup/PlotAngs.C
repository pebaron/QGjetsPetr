...
Energies.push_back("900_7000");
    ...
    for (int i = 0; i < Radii.size();i++){
        for (int j = 0; j < Lambdas.size();j++){
            for (int k = 0; k < Q.size();k++){
                quark_JetAng.push_back( TString::Format("quark_FastJets%s%sLam%s", Radii[i].Data(), Lambdas[j].Data(), Q[k].Data() ) );
                gluon_JetAng.push_back( TString::Format("gluon_FastJets%s%sLam%s", Radii[i].Data(), Lambdas[j].Data(), Q[k].Data() ) );
                mmdt_quark_JetAng.push_back( TString::Format("quark_mmdt_mmdt_FastJets%s%sLam%s", Radii[i].Data(), Lambdas[j].Data(), Q[k].Data() ) );
                mmdt_gluon_JetAng.push_back( TString::Format("gluon_mmdt_mmdt_FastJets%s%sLam%s", Radii[i].Data(), Lambdas[j].Data(), Q[k].Data() ) );
            }
        }
    }
    ...
    for (int i=0;i<vec_quark.size();i++){
        ...
        cans[i]->SaveAs(TString::Format("./angs2D/2D_%s.png", CanName.Data()));
        cans[i]->SaveAs(TString::Format("./angs2D/2D_%s.pdf", CanName.Data()));
        cans2[i]->SaveAs(TString::Format("./angs2D/2D_mmdt_%s.png", CanName.Data()));
        cans2[i]->SaveAs(TString::Format("./angs2D/2D_mmdt_%s.pdf", CanName.Data()));
