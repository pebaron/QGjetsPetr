int PlotVariableCalo(TString Variable="tag_eta",bool Logy=false ,TString TreeName="ZmumuTPMerged/Trees/IDProbes_OC/TPTree_IDProbes_OC", TString Moca="pPbMCNo0_24.root", TString Data="user.pebaron.26691297.EXT0._000029.NTUP.root"){
...
//    tree1->Print();
    ...
    //tree2->Print();
    tree1->Draw(Variable.Data(), TString::Format("%s < 100.", Variable.Data()), "Q");
    TH1 *moca_temp = (TH1*)gPad->GetPrimitive("htemp")->Clone("moca_temp"); 
    tree2->Draw(Variable.Data(), TString::Format("%s < 100.", Variable.Data()), "Q");
...
//    datapp->SetLineStyle(2);
 ...
 //   leg->AddEntry(datapp, "pp data", "l");
    ...
    if (Logy==false){
    can->SaveAs(TString::Format("./comp/%s/%s_cut100.pdf", TreeName.Data(),Variable.Data()));
    can->SaveAs(TString::Format("./comp/%s/%s_cut100.png", TreeName.Data(),Variable.Data()));
    }else{
    can->SaveAs(TString::Format("./comp/%s/%s_log_cut100.pdf", TreeName.Data(),Variable.Data()));
    can->SaveAs(TString::Format("./comp/%s/%s_log_cut100.png", TreeName.Data(),Variable.Data()));
    }
