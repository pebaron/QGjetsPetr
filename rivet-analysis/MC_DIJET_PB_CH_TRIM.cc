...
namespace Rivet {
  ...
  class MC_DIJET_PB_CH_TRIM : public Analysis {
  public:
...
//      FileName.append(".root");
 ...
 ttree->Branch("mmdt_FastJets10PtReclustQ800", &v_mmdt_ArrayOfHist[4][53]);
      ...
      _trimmer = fastjet::Filter(fastjet::JetDefinition(fastjet::kt_algorithm, 0.2), fastjet::SelectorPtFractionMin(0.01));
...
void analyze(const Event& event) {
      ...
      continue;
        ...
        for (int h=0;h<jets.size();h++){
            JetPair.push_back(_trimmer( ca_wta_recluster(jets[h]) ) );
        }
        //JetPair[0]=jet1;
        //JetPair[1]=jet2;
        ...
        for (int h=0;h<jets.size();h++){
            mmdt_JetPair.push_back(_trimmer((*mmdt)(ca_wta_recluster(jets[h])) ) );
        }
  ...
  DECLARE_RIVET_PLUGIN(MC_DIJET_PB_CH_TRIM);
