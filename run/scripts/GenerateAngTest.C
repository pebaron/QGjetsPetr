...
int GenerateAngTest(TString EnergyA = "7000", TString EnergyB = "13000", bool Pythia = false){

    TFile * rfile;

    if (Pythia){
        rfile = new TFile(TString::Format("./notrim/out_%s_%s_pythia.root", EnergyA.Data(), EnergyB.Data()), "read");
    }else{
        rfile = new TFile(TString::Format("./notrim/out_%s_%s.root", EnergyA.Data(), EnergyB.Data()), "read");
    }
    ...
    for (int k = 0; k<JetAng.size(); k++){
         if (!JetAng[k].Contains("FastJets10MultLam50")) continue;
    //for (int k = 4; k<5; k++){
    for (int i = 1; i<=fracA->GetXaxis()->GetNbins(); i++){//thinking of starting from 13th bin 52Pt
        ...
        for (int j = 9; j<=fracA->GetYaxis()->GetNbins(); j++){ //from 6 due to positive part of eta and averaging two jets pt and eta
   ...
   TFile * output;
   if (Pythia){
       output = new TFile(TString::Format("test_qg_angs_%s_%s_pythia.root", EnergyA.Data(), EnergyB.Data() ), "recreate" );
   }else{
       output = new TFile(TString::Format("test_qg_angs_%s_%s.root", EnergyA.Data(), EnergyB.Data() ), "recreate" );
   }
   ...
   output->cd();
    ...
    TH1F * h_q_mmdt;
    for (int i = 0; i<JetAng.size(); i++){
        if (!JetAng[i].Contains("FastJets10MultLam50")) continue;
    //for (int i = 4; i<5; i++){
