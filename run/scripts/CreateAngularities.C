...
int CreateAngularities(TString EnergyA = "900", TString EnergyB = "2360", bool Pythia = false){
    ...
    if (Pythia){
        ...
        fileA = new TFile(TString::Format("/disk0/QGjetsPetr/run/files_test/%s_hadr.root", EnergyA.Data() ), "read" );
        fileB = new TFile(TString::Format("/disk0/QGjetsPetr/run/files_test/%s_hadr.root", EnergyB.Data() ), "read" );
    }
    ...
    TFile * output;
    if (Pythia){
        output = new TFile(TString::Format("/disk0/QGjetsPetr/run/scripts/out_%s_%s_pythia.root", EnergyA.Data(), EnergyB.Data() ), "recreate");
    }else{
        output = new TFile(TString::Format("/disk0/QGjetsPetr/run/scripts/out_%s_%s.root", EnergyA.Data(), EnergyB.Data() ), "recreate");
    }
