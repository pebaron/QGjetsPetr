...
int GenerateAngTrim(TString EnergyA = "900", TString EnergyB = "2360", bool Pythia = false){

    TFile * rfile;

    if (Pythia){
        rfile = new TFile(TString::Format("out_trim_%s_%s_pythia.root", EnergyA.Data(), EnergyB.Data()), "read");
    }else{
        rfile = new TFile(TString::Format("out_trim_%s_%s.root", EnergyA.Data(), EnergyB.Data()), "read");
    }
   ...
   TFile * output;
   if (Pythia){
       output = new TFile(TString::Format("qg_angs_trim_%s_%s_pythia.root", EnergyA.Data(), EnergyB.Data() ), "recreate" );
   }else{
       output = new TFile(TString::Format("qg_angs_trim_%s_%s.root", EnergyA.Data(), EnergyB.Data() ), "recreate" );
   }
