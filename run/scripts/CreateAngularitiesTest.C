...
int CreateAngularitiesTest(TString EnergyA = "7000", TString EnergyB = "13000", bool Pythia = false){
    ...
    if (Pythia){
        fileA = new TFile(TString::Format("/disk0/QGjetsPetr/Petr_Pythia/%s_hadr_pythia.root", EnergyA.Data() ), "read" );
        fileB = new TFile(TString::Format("/disk0/QGjetsPetr/Petr_Pythia/%s_hadr_pythia.root", EnergyB.Data() ), "read" );
    }else{
        fileA = new TFile(TString::Format("/disk0/QGjetsPetr/run/files_test/%s_hadr_noveto.root", EnergyA.Data() ), "read" );
        fileB = new TFile(TString::Format("/disk0/QGjetsPetr/run/files_test/%s_hadr_noveto.root", EnergyB.Data() ), "read" );
    }
    ...
    TFile * output;
    if (Pythia){
        output = new TFile(TString::Format("/disk0/QGjetsPetr/run/scripts/out_%s_%s_pythia_noveto.root", EnergyA.Data(), EnergyB.Data() ), "recreate");
    }else{
        output = new TFile(TString::Format("/disk0/QGjetsPetr/run/scripts/out_%s_%s_noveto.root", EnergyA.Data(), EnergyB.Data() ), "recreate");
    }
