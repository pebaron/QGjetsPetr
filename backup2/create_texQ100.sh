...
pafilelist=""
pifilelist=""
for i in `ls *herwig_av*.pdf | sort --version-sort --field-separator=_ --reverse`; do a=`ls ../../angs1D_filter_herwig_average_no_7000_13000/Q100/*${i:10}`; ls $a;ls $i; pa=$(echo $a | sed 's\herwig_average.pdf\pythia_average.pdf\'); paa=$(ls ../../angs1D_filter_herwig_average_no_7000_13000/Q100/*${pa#*clone});ls $paa;pi=$(echo "${i:10}" | sed 's\herwig_average.pdf\pythia_average.pdf\g');pii=$(ls *$pi);ls $pii;afilelist+="${a} ";ifilelist+="${i} ";pafilelist+="${paa} ";pifilelist+="${pii} ";echo "----------------";done
...
for i in ${!afoo[@]}; do 
  ...
  #echo ${pifoo[$i]}
  #echo "----foooo------"
  var=$(echo ${afoo[$i]#*no_cut_} | sed 's\__herwig_average.pdf\\g' | sed 's&_& &g')
