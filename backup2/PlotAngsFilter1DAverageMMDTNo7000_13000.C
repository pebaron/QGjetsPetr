TH1F * GetMyHisto(TGraphAsymmErrors * a){
    int n = a->GetN();
    Double_t ax[n],ay[n];
    Double_t bins[n];
    ...
    for(Int_t i=0; i<n; i++) {
        ...
        bins[i] = ax[i] - a->GetErrorXlow(i);
        if (i==n-1) bins[i+1] = ax[i] + a->GetErrorXhigh(i);
    }

    TH1F * h = new TH1F(TString::Format("myh_%s", a->GetName()), "", n, bins);
    for(Int_t i=0; i<n; i++) {
        a->GetPoint(i,ax[i],ay[i]); 
        h->SetBinContent(i+1, ay[i]);
        h->SetBinError(i+1, 0.5*(a->GetErrorXlow(i) + a->GetErrorXhigh(i)) );
    }
...
double SeparationPower(TGraphAsymmErrors * a, TGraphAsymmErrors * b){
    ...
    for(Int_t i=0; i<n; i++) {
        ...
        //if (ay[i]<0) ay[i] = 0.;
        //if (by[i]<0) by[i] = 0.;
        if (ay[i]> 0) IntegralA+=ay[i];//*(a->GetErrorXlow(i)+a->GetErrorXhigh(i) );
        if (by[i]> 0) IntegralB+=by[i];//*(b->GetErrorXlow(i)+b->GetErrorXhigh(i) );
    }
    cout << IntegralA << endl;
    cout << IntegralB << endl;
    for(Int_t i=0; i<n; i++) {
...
int PlotAngsFilter1DAverageMMDTNo7000_13000(int Pythia = 0, TString QPar = "50"){
...
Energies.push_back("900_7000");
    ...
    for (int i = 0; i < Radii.size();i++){
        for (int j = 0; j < Lambdas.size();j++){
            for (int k = 0; k < Q.size();k++){
             mmdt_quark_JetAng.push_back( TString::Format("quark_clone_h_no_cut_FastJets%s%sLam%s", Radii[i].Data(), Lambdas[j].Data(), Q[k].Data() ) );
             mmdt_gluon_JetAng.push_back( TString::Format("gluon_h_no_cut_FastJets%s%sLam%s", Radii[i].Data(), Lambdas[j].Data(), Q[k].Data() ) );
             quark_JetAng.push_back( TString::Format("quark_clone_h_no_cut_mmdt_FastJets%s%sLam%s", Radii[i].Data(), Lambdas[j].Data(), Q[k].Data() ) );
             gluon_JetAng.push_back( TString::Format("gluon_h_no_cut_mmdt_FastJets%s%sLam%s", Radii[i].Data(), Lambdas[j].Data(), Q[k].Data() ) );
    ...
    TFile * output = new TFile("/media/petr/DATADRIVE1/Users/petrb/Documents/angs_herwig1D_average_mmdt_no7000_13000.root", "recreate");
    output->cd();
...
cout << "test1" << endl;
        ...
        mygluon = GetMyHisto(average_gluon);
        myquark = GetMyHisto(average_quark);
        mygluon_no_MPI = GetMyHisto(average_No_MPI_gluon);
        myquark_no_MPI = GetMyHisto(average_No_MPI_quark);
        ...
        //leg->AddEntry((TObject*)0, TString::Format("#Chi^{2}/NDF quark = %0.2f", myquark->Chi2Test(myquark_no_MPI, "CHI2/NDF") ), "");
        //leg->AddEntry((TObject*)0, TString::Format("#Chi^{2}/NDF gluon = %0.2f", mygluon->Chi2Test(mygluon_no_MPI, "CHI2/NDF") ), "");
        //leg->AddEntry((TObject*)0, TString::Format("#Delta_{#Chi^{2}} = %0.2f", power / (myquark->Chi2Test(myquark_no_MPI, "CHI2/NDF")) * mygluon->Chi2Test(mygluon_no_MPI, "CHI2/NDF") ), "");
